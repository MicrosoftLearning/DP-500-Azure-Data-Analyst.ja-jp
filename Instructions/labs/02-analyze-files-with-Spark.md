...
4. **[Manage]** ページで、 **[Apache Spark pools]** タブを選択し、**spark*xxxxxxx*** のような名前の Spark プールがワークスペース内にプロビジョニングされていることに注意してください。 この Spark プールは、ワークスペースのデータ レイク ストレージ内のファイルからデータを読み込んで分析するために後で使用します。
5. **[Data]** ページで **[Linked]** タブを表示して、**synapse*xxxxxxx* (Primary - datalake*xxxxxxx*)** のような名前の Azure Data Lake Storage Gen2 ストレージ アカウントへのリンクがワークスペースに含まれていることを確認します。
...
1. **orders** フォルダー内でいずれかのファイルを選択したら、ツールバーの **[新しいノートブック]** の一覧で **[DataFrameに読み込む]** を選択します。 データフレームは、表形式データセットを表す Spark の構造です。
...
3. セルの下の結果セクションで、 **[表示]** オプションを **[テーブル]** から **[グラフ]** に変更します。
4. グラフの右上にある **[表示オプション]** ボタンを使用して、グラフのオプション ペインを表示します。 オプションを次のように設定し、 **[適用]** を選択します。
    - **グラフのタイプ**: 横棒グラフ
    - **キー**: Item
    - **値**: Quantity
    - **系列グループ**: 空白のままにする
