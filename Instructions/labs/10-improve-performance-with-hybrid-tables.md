...
11. このタスクでは、**Sales** クエリにフィルターを追加します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image32.png)

### <a name="filter-the-query"></a>**[クエリ]** ペインで、 **[Sales]** クエリを選択します。

**OrderDate** 列のヘッダーで、下矢印を選択し、 **[日付/時刻フィルター]**  >  **[次の値の間]** を選択します。

1. **[行のフィルター処理]** ウィンドウで、最初のカレンダー アイコンのドロップダウン リストを選択し、 **[パラメーター]** を選択します。

2. 隣接するドロップダウン リストで、**RangeStart** パラメーターが設定されていることに注意してください。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image33.png)

3. "既定のパラメーター選択は正しい選択です。"**

    ![](../images/dp500-improve-performance-with-hybrid-tables-image34.png)

4. 2 番目の [範囲] ドロップダウン リストで、 **[次の日付より前]** を選択します。

    対応するドロップダウン リストで、 **[RangeEnd]** パラメーターを選択します。

5. **[OK]** を選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image35.png)

6. **[ホーム]** リボン タブの **[閉じる]** グループ内から、 **[閉じて適用]** アイコンをクリックします。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image36.png)

7. Power BI Desktop によって新しいテーブルに 5,134 行が **Sales** テーブルに読み込まれたことを確認します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image37.png)

8. "これらは、2022 年 6 月でフィルター処理された行です。"**
...
10. このタスクでは、**Sales** テーブルの増分更新ポリシーを設定します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image40.png)

### <a name="set-up-incremental-refresh"></a>モデル図で、**Sales** テーブルのヘッダーを右クリックし、 **[増分更新]** を選択します。

**[増分更新とリアルタイム データ]** ウィンドウの手順 2 で、増分更新をオンにします。

1. 更新日の **2 年**前からデータのアーカイブを開始するように設定します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image41.png)

2. "この設定により、履歴期間が決まります。この場合、Power BI では履歴データ用に 2 つの年間パーティションを作成します。"**

    ![](../images/dp500-improve-performance-with-hybrid-tables-image42.png)

3. 更新日の **7 日**前からデータの増分更新を開始するように設定します。
...
4. 手順 3 で、 **[DirectQuery を使用して最新のデータをリアルタイムで取得する]** オプションをオンにします。
...
7. このタスクでは、データセットを発行します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image47.png)

### <a name="publish-the-dataset"></a>レポートを発行するには、 **[ホーム]** リボン タブの **[発行]** を選びます。

**[Power BI に発行]** ウィンドウで、このラボで作成したワークスペースを選択し、[選択] を選択します。
...
3. 変更を保存するかどうかを確認するウィンドウで、 **[保存]** を選びます。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image50.png)

4. データセットを設定する

5. このタスクでは、データ ソースの資格情報を設定し、データセットを更新します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image51.png)

### <a name="set-up-the-dataset"></a>Power BI サービス Web ブラウザー セッションに切り替えます。

ワークスペースのランディング ページで、レポートとデータセットを見つけます。

1. データセットをカーソルでポイントし、省略記号が表示されたら、省略記号を選んで、 **[設定]** を選択します。

2. **[データ ソースの資格情報]** セクションで、 **[資格情報の編集]** リンクを選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image52.png)

3. このウィンドウで、ユーザー名とパスワードを入力し、プライバシー レベルを [組織] に設定します。
...
5. パスワード: `P@ssw0rd01`
    ...
    **[スケジュールされた更新とパフォーマンスの最適化]** セクションを展開して開きます。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image54b.png)

6. 確認するだけで、どの設定も変更しないでください。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image55.png)

8. "実際の設定では、Power BI でパーティションを定期的に更新および管理できるようにデータの更新をスケジュールします。"**

    ![](../images/dp500-improve-performance-with-hybrid-tables-image56.png)

9. "このラボでは、オンデマンド更新を実行します。"**

    **[ナビゲーション]** ペイン (左側にあります) で、お使いのワークスペースを選択します。

    ワークスペース ランディング ページで、データセットをカーソルでポイントし、 **[更新]** アイコンを選択します。

10. **Refreshed** 列で、回転するアイコンを確認し、それが停止するまで待ちます (停止すると、更新が完了したことを示します)。

11. ワークスペースの設定を開くには、右上にある **[設定]** を選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image57.png)

12. **[設定]** ペインで、 **[Premium]** タブを選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image58.png)

13. ワークスペース接続をクリップボードにコピーするには、 **[コピー]** を選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image59.png)

14. "SQL Server Management Studio (SSMS) で、このワークスペース接続を使用してワークスペースに接続します。"**

    ![](../images/dp500-improve-performance-with-hybrid-tables-image60.png)

15. ペインを閉じるには、 **[キャンセル]** を選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image61.png)

    テーブル パーティションを確認する

16. このタスクでは、SSMS を使用してテーブル パーティションを確認します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image62.png)

### <a name="review-the-table-partitions"></a>SSMS を開くには、タスク バーで、**SSMS** のショートカットを選択します。

**[サーバーに接続]** ウィンドウの **[サーバーの種類]** ドロップダウン リストで、 **[Analysis Services]** を選択します。

1. "XMLA 読み取り/書き込みエンドポイントを使用すると、SSMS を使用してワークスペースに接続できます。このエンドポイントは、Premium ワークスペースでのみ使用できます。"**

    ![](../images/dp500-improve-performance-with-hybrid-tables-image63.png)

2. **[サーバー名]** ボックスで、ワークスペース接続を貼り付けて (**Ctrl + V** キーを押して) テキストを置き換えます。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image64.png)

    **[認証]** ドロップダウン リストで、 **[Azure Active Directory - パスワード]** を選択します。

3. ラボの資格情報を入力します。

4. **[接続]** を選択します。

5. オブジェクト エクスプローラー (左側にあります) で、**Databases** フォルダー、 **[Sales Analysis...]** データベース (データセット)、**Tables** フォルダーの順に展開して開きます。

6. **Sales** テーブルを右クリックし、 **[パーティション]** を選択します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image65.png)

7. **[パーティション]** ウィンドウで、2 年間の履歴のパーティション、その後に続く四半期パーティションと日単位のパーティションの一覧を確認します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image66.png)

8. 一覧を下方にスクロールし、最後のパーティションが、現在および将来の日付の DirectQuery パーティションであることを確認します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image67.png)

9. "Power BI では、これらのすべてのパーティションが自動的に作成および管理されます。"**

10. **[キャンセル]** を選択します。

    ハイブリッド テーブルをテストする

11. この演習では、レポートを開き、販売注文を追加し、レポート データの更新を確認します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image68.png)

## <a name="test-the-hybrid-table"></a>レポートを開く

このタスクでは、レポートを開きます。

### <a name="open-the-report"></a>Power BI サービス Web ブラウザー セッションに切り替えます。

ワークスペース ランディング ページで、レポートを選択します。

1. 必要に応じて、 **[会計年度]** スライサーで、現在の月 (今日の日付に基づく) を含む会計年度を選択します。

2. "現在の月は、横棒グラフの横棒として表示されます。"**
...
### <a name="add-an-order-to-the-database"></a>スクリプト ファイルを開くには、 **[ファイル]** メニューで、 **[開く]**  >  **[ファイル]** を選択します。

**[ファイルを開く]** ウィンドウで、**D:\DP500\Allfiles\10\Assets** フォルダーに移動します。

1. **1-InsertOrder.sql** ファイルを選択し、 **[開く]** を選択します。

2. **[データベース エンジンに接続]** ウィンドウで、 **[サーバー名]** ドロップダウン リストがラボの Azure SQL Database サーバーに設定されていることを確認します。

3. **[認証]** ドロップダウン リストで、 **[Azure Active Directory - パスワード]** を選択します。
...
7. "このスクリプトでは、今日を注文日として使用して、1 件の注文を **FactInternetSales** テーブルに挿入します。"**

8. スクリプトを実行するには、ツール バーで **[実行]** を選択します (または **F5** キーを押します)。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image71.png)

9. ファイルを閉じるには、 **[ファイル]** メニューで **[閉じる]** を選択します。

    レポートを更新する

10. このタスクでは、レポートを更新します。

    ![](../images/dp500-improve-performance-with-hybrid-tables-image72.png)

11. Power BI サービス Web ブラウザー セッションに切り替えます。

### <a name="refresh-the-report"></a>レポートで、現在の月の売上金額をメモします。

アクション バーで、 **[更新]** コマンドを選択します。

1. レポートの更新が完了したら、現在の月の売上金額が 10,000 ドル増加したことを確認します。

2. "Power BI では **Sales** テーブルのクエリを実行したときに、Azure SQL データベースに直接クエリを実行した DirectQuery パーティションから現在のデータを取得しました。"**

3. "ヒント: ハイブリッド テーブルは、ページの自動更新 (Power BI レポートを自動的に更新する機能) を使用する場合に特に適しています。"**
...
4. 仕上げ
    ...
    SSMS で、**2-Cleanup.sql** ファイルを開きます。

### <a name="finish-up"></a>このスクリプトは、挿入した注文を削除します。

スクリプトを実行します。 SSMS を閉じます。
